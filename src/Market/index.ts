...
const tick = async () => {
	//this is serious need of caching
...
app.get("/api/member/:id", (req, res) => {
	...
	if (id && cells.has(id)) res.json(cells.get(id));
...
app.post("/api/member/", async (req, res) => {
	...
	const data = req.body;
	if (
		Object.keys(data).filter((k) => format.some((e) => k === e)).length ===
		format.length
	) {
		...
		cells.set(id, cell);
		await document();
	} else res.status(400).json({ message: "invalid format!", format: format });
});
...
async function document() {
	try {
		...
		if (!entry) {
			...
			await DB.Models.Market.findByIdAndUpdate(this.id, body, {
				upsert: true,
			});
		}
	} catch (error) {
