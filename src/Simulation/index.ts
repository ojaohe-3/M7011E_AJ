...
app.get('/api/members', (req, res) => {
    const data = {
        consumers: Array.from(Simulator.singelton.consumers.values()),
        prosumers: Array.from(Simulator.singelton.prosumers.values()),
        managers: Array.from(Simulator.singelton.managers.values())
    }
    res.json(data);
});
...
async function fetchAll() {
    try { // todo fix initial condition, in case of newly generated nodes
        ...
        const sim = new Simulator({
            lat: + sim_data.lat,
            lon: + sim_data.lon
        }, sim_data.manager_dest, sim_data.prosumer_dest);
